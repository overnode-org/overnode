...
start_etcd() {
    ...
    etcd --name clusterlite-etcd-${current_id} --data-dir=/data \
...
init_and_start_etcd() {
    ...
    etcd --name clusterlite-etcd-1 --data-dir=/data \
        --listen-peer-urls http://${CONTAINER_IP}:2380 \
        --listen-client-urls http://${CONTAINER_IP}:2379,http://127.0.0.1:2379 \
        --advertise-client-urls http://${CONTAINER_IP}:2379 \
        --initial-advertise-peer-urls http://${CONTAINER_IP}:2380 \
...
join_and_start_etcd() {
    ...
    etcdctl --endpoints=${endpoints} member add clusterlite-etcd-${current_id} http://${CONTAINER_IP}:2380
    start_etcd
}
