...
echo "[cade elasticsearch] starting..."
...
if [ -z "$SERVICE_SEEDS" ];
then
    echo "[cade elasticsearch] the service requires declaration of seeds option in the placements section of the configuration, exiting..."
    exit 1
fi
...
echo "cluster.name: cadeworks" >> $config_target
...
echo "[cade elasticsearch] starting elasticsearch on ${CONTAINER_IP}"
echo "[cade elasticsearch] with configuration ${config_target}:"
cat ${config_target}
